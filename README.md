...
| 领域             | 能力说明                                   | 业务价值                         |
| ---------------- | ------------------------------------------ | -------------------------------- |
| 🟢 Core 生命周期 | `Start` / `Stop` / `Restart`、高内存守护   | 快速恢复网络可用性，降低中断时间 |
...
| 📊 可观测性      | 实时速率、连接数、内存、活动连接、内存日志 | 快速定位“慢、断、不通”等问题     |
| 🧯 流量熔断      | 后台识别 TUN 异常放大并定向关闭单连接      | 抑制连接风暴，不改用户网络配置   |
...
- 🧭 `Proxy`：实时速率、连接数、内存与守护状态、配置入口、系统代理开关
- 📚 `Rules`：规则统计、规则刷新、Provider 更新
- 🌐 `Connections`：连接过滤、关闭单连接、关闭全部连接
- 🪵 `Logs`：日志级别过滤、关键词检索、日志复制
- ⚙️ `System`：语言、状态栏样式、核心内存控制、`allow-lan` / `ipv6` / `log-level`、端口设置
...
| `logs/`   | 历史版本日志目录；当前版本不再写入本地日志文件     | 如有旧日志文件，可按需手动清理   |
...
ClashBar 提供一个本地自愈型的 **核心内存控制** 设置，用于降低 `mihomo` 长时间运行后内存异常增长带来的影响。
...
- 该设置只对本地 core 生效，不对远端控制目标生效。
- 开启后，ClashBar 会在本地 core 运行期间保持 `/memory` 数据流，以便后台也能检测内存。
- 当 `mihomo` 当前内存达到所选阈值时，ClashBar 会写入一条内存日志，并复用现有 `Restart Core` 流程自动重启。
- 自动重启触发后有固定 10 分钟冷却时间，避免异常配置或短时间峰值导致连续重启。
- Proxy 页面左下角仍显示实时内存值；开启守护时会用有色盾牌图标提示当前处于受保护状态。

> [!NOTE]
> 内存守护是 ClashBar 本地保护策略，不会修改 `mihomo` 配置，也不会同步到远程机器。

## 🧯 TUN 异常流量自动熔断

本地 core 运行且运行时 TUN 已启用时，ClashBar 会在后台自动检测“进入 TUN 的包量远高于实际外发流量”的异常放大现象；菜单面板关闭时仍然生效。该保护只依赖兼容的 `mihomo` Controller API，不要求修改用户 YAML，也不限制用户替换 custom core。

触发过程：

1. 空闲状态下约每 10 秒开启一个 1 秒采样窗口；TUN 超过 `10,000 PPS` 后锁定异常方向。
2. 锁定方向必须连续 3 个约 1 秒样本同时满足：
   - TUN 仍超过 `10,000 PPS`；
   - TUN PPS 至少达到其他接口同方向 PPS 的 8 倍，并额外高出 5,000 PPS；
   - core 同方向有效载荷至少为 `64 KiB/s`；
   - TUN 字节速率至少为 core 有效载荷速率的 1.5 倍。
3. 触发后读取完整 `/connections` 快照，在连续两个归因窗口中寻找同一个稳定候选。候选必须是 TUN 连接、方向增量至少 `64 KiB`、唯一第一名且占同方向 TUN 增量至少 80%，同时保持相同的 tracker UUID、启动时间和绝对进程路径。
4. ClashBar、`mihomo` 以及完整路径中含 `clash` / `mihomo` 的进程会被保护；连接快照不完整、候选不唯一或身份发生变化时不执行恢复动作。
5. 归因成功后只调用 `DELETE /connections/<tracker-uuid>` 关闭该精确连接，不执行关闭全部连接。关闭后还会确认 UUID 已消失、同方向 TUN PPS 已降至关闭前的 20% 以下，并且异常守恒条件已经解除。

恢复限速采用**滚动 30 秒窗口**：

- 任意滚动 30 秒内最多发起 3 次精确连接关闭；这不是应用整个运行期间的总次数上限。
- 旧尝试超过 30 秒后会自动移出窗口并恢复额度，因此反复重连的异常程序仍会继续被检测和定向处理。
- 只有已经完成归因、即将发送精确 DELETE 的动作才占用额度；普通采样失败或无法确定唯一候选不计次数。
- 达到窗口上限后暂不执行第 4 次 DELETE，并进入短暂冷却；不会为了继续处理而扩大到其他应用或全局网络状态。
...
- 不关闭全部连接；
- 不修改 YAML、运行时配置或用户规则；
- 不自动添加 `BLOCK` / `DIRECT` 规则；
- 不创建新的本地数据或日志文件，诊断信息只保留在当前会话内存中；
- 仅对本地 core 生效，不对远端控制目标执行自动连接关闭。

> [!IMPORTANT]
> 该机制优先避免误伤其他应用：如果无法在两个连续归因窗口中证明某一个连接稳定且占绝对主导，ClashBar 会保持现状，不会猜测性关闭连接。

## 🪵 日志策略

当前版本取消了 ClashBar 本地日志文件写入：

- 不再创建或追加 `~/Library/Application Support/clashbar/logs/clashbar.log`
- 不再创建或追加 `~/Library/Application Support/clashbar/logs/mihomo.log`
- `Clear Logs` 只清空内存日志，不会创建、清空或截断磁盘文件
- 既有 `logs/` 目录和历史日志文件可能仍然存在，但当前版本不会继续写入

仍然保留的能力：

- ClashBar 自身日志继续进入内存日志列表
- `mihomo` stdout/stderr 继续进入内存日志列表
- 打开 Logs 页面时，仍可查看当前会话内最近日志
- 其他外部面板（例如 zashboard）仍可直接连接 `mihomo` 的 `/logs` 能力，不依赖 ClashBar 本地文件

> [!IMPORTANT]
> 日志现在只保存在当前会话内存中。应用退出后，ClashBar 不再保留本地日志文件；如果需要长期留存，请使用外部采集方式。
...
### 6) 为什么 `logs/` 目录还在，但日志文件不再增长？ 🪵

**现象**：`~/Library/Application Support/clashbar/logs/` 目录仍存在，但 `clashbar.log` / `mihomo.log` 不再更新。  
**原因**：当前版本保留统一工作目录结构，但已经切断本地日志文件写入路径。

**说明**

1. 目录存在不代表当前版本会写入日志文件。
2. 如果里面还有旧的 `clashbar.log` / `mihomo.log`，它们是历史遗留文件，可按需删除。
3. 当前日志只在应用运行期间保存在内存里，用于 UI 展示和当次排障。
