...
- 稳定：
  ...
  - 可选启用本地核心内存控制，在 `mihomo` 内存达到固定阈值时自动复用重启流程。
- 可观测：
  - 展示实时上下行速率。
  - 展示连接数量、连接列表、规则命中、日志与 Provider 状态。
  - 保留 ClashBar 自身日志与 mihomo 日志的内存展示，并做脱敏处理。
  - 当前版本不再把运行日志写入本地磁盘文件。
- 原生：
  - 使用 macOS 菜单栏交互模型。
...
| 设置维护        | 语言、外观、状态栏样式、核心内存控制、日志等级、端口、缓存清理 | 支持直接修改部分 `mihomo` 配置并持久化 UI 状态，本地内存守护不进入 `/configs` |
...
- 一键复制终端环境变量脚本
  - 判断 mixed-port 还是 单独 http port 和 socks port
- 复制结果包含：
  - `https_proxy`
  - `http_proxy`
  - `all_proxy`
